...
class App {  
    ...
    constructor () {
        ...
        this.authRouter = new AuthRouter(Logger.create, appConfig);
    ...
    private async mountRoutes (): Promise<void> {
        ...
        router.options('*');
        router.post('/api/user/googletoken', express.text({ limit: '1MB', type: '*/*'}) , (req, res) => this.authRouter.loginGoogle(req, res));
