resource "aws_lambda_function" "lam-rs-dbdeployer" {
  ...
  handler       = "app/index.createHandler"
  memory_size = 128
  timeout = 15
  publish = "true"
...
resource "aws_lambda_function" "lam-rs-dbdestroyer" {
  s3_bucket = "labar.jimbo.code"
  s3_key = "build/rs.db.zip"
  function_name = "${var.env_prefix}-rs-dbdestroyer"
  role          = aws_iam_role.rs-iamrole-dbdeployer.arn
  handler       = "app/index.deleteHandler"
  memory_size = 128
  timeout = 15
  publish = "true"
...
resource "aws_lambda_function" "lam-rs-dbresetter" {
  s3_bucket = "labar.jimbo.code"
  s3_key = "build/rs.db.zip"
  function_name = "${var.env_prefix}-rs-dbresetter"
  role          = aws_iam_role.rs-iamrole-dbdeployer.arn
  handler       = "app/index.resetHandler"
  memory_size = 128
  timeout = 15
  publish = "true"
